...
export default function AllPerks() {
  ...
  // Load all perks (with optional filters)
  async function loadAllPerks() {
    setError('')
    setLoading(true)
    try {
  ...
  // Effect #1 — Load all perks on mount
  useEffect(() => {
    loadAllPerks()
  }, [])

  // Effect #2 — Auto-search when query or merchant changes
  useEffect(() => {
  ...
  // Effect #3 — Extract unique merchants
  useEffect(() => {
    const merchants = perks
      .map(p => p.merchant)
      .filter(m => m && m.trim())
